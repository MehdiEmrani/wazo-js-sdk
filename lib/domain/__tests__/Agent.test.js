...
describe('Call Center Agent domain', () => {
    it('should keep its values', () => {
        // @ts-expect-error
        const agent = new Agent_1.default(formatted);
        expect(agent).toBeInstanceOf(Agent_1.default);
        Object.keys(fieldMap).map(key => expect(agent[key]).toBe(formatted[key]));
    });
    it('can parse a plain call center agent to domain', () => {
        const agent = Agent_1.default.parse(plain);
        // @ts-expect-error
        expect(agent).toEqual(new Agent_1.default(formatted));
    });
});
